...
from avsUI.freenas import forms, models, utils
...
def start(request):
    avs_key, avs_secret = utils.get_avs_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=avs_key,
        secret=avs_secret)
    ...
    try:
        avs = models.AVS.objects.order_by('-id')[0]
        avs.enable = True
        avs.save()
    except IndexError:
        avs = models.AVS.objects.create(enable=True)

    try:
        form = forms.AVSForm(avs.__dict__, instance=avs, jail=jail)
        form.is_valid()
        form.save()
    except ValueError:
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': 'AVS data did not validate, please configure it first.',
    ...
    cmd = "%s onestart" % utils.avs_control
    _popen = os.popen(cmd)
...
def stop(request):
    ...
    try:
        ...
        avs = models.AVS.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.avs_control
    pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
...
def edit(request):
    ...
    if request.method == "GET":
        form = forms.AVSForm(instance=avs,
            jail=jail)
    ...
    form = forms.AVSForm(request.POST,
        instance=avs,
        jail=jail)
    if form.is_valid():
        form.save()
        return JsonResponse(request, error=True, message="AVS settings successfully saved.")
...
def treemenu(request):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('avs_edit'),
        'kwargs': {'plugin_name': 'avs'},
    }
...
def status(request):
    ...
    proc = Popen(["/usr/bin/pgrep", "-U", "root", "-F", "/var/run/avs.pid"], stdout=PIPE, stderr=PIPE)
...
def treemenu_icon(request):
    with open(utils.avs_icon, 'rb') as f:
        icon = f.read()
